...
Субъективная обратная связь в текущей архитектуре рассматривается как отдельный ground truth / calibration layer: она записывается после расчета readiness и recommendation, хранит historical context snapshot и не вмешивается в deterministic core.
...
- subjective feedback (`post_ride_rpe`, `next_day_recovery`) сохраняется отдельно и не меняет deterministic расчеты
